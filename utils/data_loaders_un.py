...
class ShapeNetDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        taxonomy_name, sample_name, rendering_images, volume = self.get_datum(idx % len(self.file_list))
        unlabel_name,_,rendering_unlabel, _ = self.get_datum_un(idx % len(self.file_list_un))
        ...
        if self.dataset_type == DatasetType.TRAIN:
            ...
            rendering_unlabel_strong = self.strong_transforms(rendering_unlabel)

            return taxonomy_name, sample_name, rendering_source,unlabel_name, rendering_unlabel_slight,\
                   rendering_unlabel_strong, volume
        else:
            rendering_source = self.slight_transforms(rendering_source)
            return taxonomy_name, sample_name, rendering_source, volume
    ...
    def get_datum_un(self, idx):
        ...
        volume_path = self.file_list_un[idx]['volume']
...
class Pix3dDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_type == DatasetType.TRAIN:
            taxonomy_name, sample_name, rendering_images, volume, bounding_box = self.get_datum(
